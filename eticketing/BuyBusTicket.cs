...
{
    public partial class BuyBusTicket : MetroForm
    {
        ...
        public BuyBusTicket(string userid)
        {
            InitializeComponent();
            this.Da = new DataAccess();
            UserName = userid;
        ...
        private void fillTime()
        {
            ...
            string s1 = @"select DISTINCT departuretime from scheduleinfo where destination='" + comboBox2.Text+"'and departure='"+comboBox1.Text+"'and date='"+dateTimePicker1.Text+"';";
            Ds = Da.ExecuteQuery(s1);
            for (int i = 0; i < this.Ds.Tables[0].Rows.Count; i++)
            {
        ...
        private void btnNext_Click(object sender, EventArgs e)
        {
            ...
            if (Int32.Parse(dateTimePicker1.Text.Substring(0, 2)) == DateTime.Now.Day)
            {
                ...
                {
                    BustSeatPicker bsp = new BustSeatPicker(UserName, dateTimePicker1.Text, metroComboBox1.Text, comboBox1.Text, comboBox2.Text);
                    bsp.Visible = true;
                    this.Visible = false;
                }
                else
                {
                    lbltime.Text = "Please Select a time grater than " + DateTime.Now.Hour.ToString();
                }
            }
            else
            {
        ...
        private void BuyBusTicket_Load(object sender, EventArgs e)
        {
